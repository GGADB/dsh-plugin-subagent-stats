...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		/** Build the display groups for one stats/usage pair (mirrors the shipped strip). */
		...
		*/
		function createMergedStatsLine(sessions) {
			return function MergedStatsLine({ useSession, useProjection, useSessions, sessionId, t }) {
				...
				// Truncation detection: the tooltip appears only when the line
				// overflows its box (the shipped StatsLine behavior). Hooks stay
				...
				// Tooltip: full merged line when truncated, plus the main-vs-
				// sub-agents breakdown whenever sub-sessions exist. Self-drawn
				// bubble (no 50vw wrap cap) so every row stays on one line.
				const showTooltip = truncated || merged.descendantCount > 0;
				const showBubble = () => {
				...
				const rows = [];
				const pushRow = (label, groupsText, sub) => {
					rows.push(react.createElement("div", { key: rows.length, style: TT_ROW },
						react.createElement("span", { style: TT_LABEL }, label),
						react.createElement("span", { style: sub ? TT_SUB : TT_MERGED },
							groupsText === "" ? t("stats.tooltip.empty") : groupsText
						)
					));
				};
				if (truncated) pushRow(t("stats.tooltip.merged"), line, false);
				if (merged.descendantCount > 0) {
					rows.push(react.createElement("div", { key: "div", style: TT_DIVIDER }));
					pushRow(t("stats.tooltip.root"), groupsToLine(buildGroups(t, rootStats, usage)), true);
					pushRow(
						t("stats.tooltip.children", { count: merged.descendantCount }),
						groupsToLine(buildGroups(t, merged.childStats, merged.childUsage)),
						true
					);
					...
					if (merged.children.length <= 5) {
						for (const child of merged.children) {
							...
							pushRow(
								`· ${short}`,
								groupsToLine(buildGroups(t, child.stats ?? ZERO_STATS, child.usage)),
								true
							);
						}
					}
				...
				return react.createElement(react.Fragment, null,
					react.createElement("div", {
						...
						groups.map((group, i) => react.createElement(
							react.Fragment,
							{ key: i },
