...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		// Emphasized key values (cache hit %, input/output tokens).
		const TT_STRONG = {
			color: "#ffffff",
			fontWeight: 600,
		...
		// Distinct title colors: merged / main session / sub-agents.
		const TT_TITLE_MERGED = "#ffd166";
		...
		*/
		function createMergedStatsLine(sessions) {
			return function MergedStatsLine({ useSession, useProjection, useSessions, sessionId, t }) {
				...
				const pushRow = (label, labelColor, groups, sub) => {
				...
				if (truncated) pushRow(t("stats.tooltip.merged"), TT_TITLE_MERGED, groups, false);
				if (merged.descendantCount > 0) {
					rows.push(react.createElement("div", { key: "div", style: TT_DIVIDER }));
					pushRow(t("stats.tooltip.root"), TT_TITLE_ROOT, buildGroups(t, rootStats, usage), true);
					pushRow(
