...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		* alignment). Cache-hit / input / output values are flagged (`strong`).
		*/
		...
		// Page text hierarchy: labels are secondary, values primary (theme-aware).
		const TT_LABEL = {
			color: "var(--dsw-alias-label-secondary)"
		};
		// Emphasized key values (cache hit %, input/output tokens): the DeepSeek
		// brand blue on a soft chip — the page's own highlight language.
		const TT_STRONG = {
		...
		// Distinct title colors from the page palette: brand blue for the main
		// session, the tools-tint violet (used by the page's context meter) for
		// sub-agents.
		const TT_TITLE_ROOT = "var(--dsw-static-deepseek-450)";
		...
		*/
		function createMergedStatsLine(sessions) {
			return function MergedStatsLine({ useSession, useProjection, useSessions, sessionId, t }) {
				...
				const visibleSlots = [];
				for (let si = 0; si < slots.length; si++) if (slots[si].length > 0) visibleSlots.push({ si, parts: slots[si] });
