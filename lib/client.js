...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = ".dsh-subagent-stats-root{text-align:center;max-width:var(--dsh-chat-content-width);box-sizing:border-box;width:100%;padding:4px calc(var(--dsh-composer-side-clearance) + 16px) 0px;color:var(--dsw-alias-label-primary);white-space:nowrap;margin:0 auto;font-size:12px;line-height:20px;display:block}.dsh-subagent-stats-sep{color:var(--dsw-alias-separator-primary);margin:0 10px}.dsh-subagent-stats-strong{color:var(--dsw-static-deepseek-400);font-weight:600}";
		...
		*/
		...
		// Distinct title colors from the page palette: the brighter brand blue
		// for the main session, the tools-tint violet (used by the page's
		// context meter) for sub-agents.
		const TT_TITLE_ROOT = "var(--dsw-static-deepseek-400)";
		...
		*/
		function createMergedStatsLine(sessions) {
			return function MergedStatsLine({ useSession, useProjection, useSessions, sessionId, t }) {
				...
				// hover bubble keeps the full wording.
				const stripSlots = buildGroups(t, merged.stats, merged.usage, true);
