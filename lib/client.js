		...
		*/
		...
		function SettingsCard({ t }) {
			...
			return react.createElement("div", { style: CARD_STYLE },
				react.createElement("div", { style: CARD_TITLE }, t("settings.title")),
				react.createElement("div", { style: CARD_DESC }, t("settings.desc")),
		...
		// ── settings section: a second-level entry in the settings nav ─────────
		// ("统计条" tab), so the full option list lives in its own panel instead
		// of crowding the General section.
		const SECTION_WRAP = { display: "flex", flexDirection: "column", width: "100%" };
		function SubagentStatsSection({ renderSlot }) {
			return react.createElement("div", { style: SECTION_WRAP }, renderSlot("settings.subagentstats.item", {}));
		}
		...
		function apply(ctx) {
			...
			// Second-level settings menu entry (tab) with its own panel.
			ctx.slots.inject("settings.section", () => ctx.slots.register({
				...
				locale: NS,
				children: {
					"settings.subagentstats.item": {
						kind: "list",
						scope: "root"
					}
				}
			}, SubagentStatsSection));
			ctx.slots.inject("settings.subagentstats.item", () => ctx.slots.register({
				name: "settings.subagentstats.item",
